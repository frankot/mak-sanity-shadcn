    ...
    <div className="bg-stone-800 py-10">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="mx-auto max-w-2xl ">
          ...
          <p className="mt-2 text-lg leading-8 text-stone-100 font-semibold">
            Szkolenia, zajęcia z instruktorem, sekcje wspinaczkowe i kursy{" "}
          </p>
        </div>
        <div className="mx-auto mt-16 grid max-w-2xl auto-rows-fr grid-cols-1 sm:grid-cols-2 gap-8 sm:mt-20 lg:mx-0 lg:max-w-none lg:grid-cols-3">
          {posts.slice(0, 6).map((post) => (
            <article
              key={post.id}
              className="relative isolate group flex flex-col cursor-pointer justify-end overflow-hidden rounded-3xl bg-gray-900 px-8 pb-8 pt-80 sm:pt-48"
            >
        ...
        <button className="flex justify-center text-white  mx-auto px-4 py-2 rounded-3xl text-main font-bold tracking-wider uppercase my-10 bg-orange-400 hover:translate-y-1 hover:shadow-lg duration-200">
          Wszystkie
        </button>
      </div>
    </div>
